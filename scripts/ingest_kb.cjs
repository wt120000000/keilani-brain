...
async function upsertChunk({title,source,chunk,embedding,idx,total}){
  const {error}=await supa.from("kb_chunks").insert({
    title,source,chunk,embedding,metadata:{idx,total,source,title}
...
(async function main(){
  ...
  if(!files.length){ console.log("No files matched:",pattern); process.exit(0); }
  ...
  for(const f of files){
    ...
    let i=0; for(const chunk of chunks){ i++;
