...
exports.handler = async (event) => {
  ...
  let audioBase64 = "", mimeHint = "", language;
  try {
  ...
  let buf;
  try { buf = Buffer.from(audioBase64, "base64"); }
  catch (e) { return json(400, { error: "bad_base64", detail: String(e.message || e) }); }
  ...
  const fileName =
    ...
    finalMime.includes("webm") ? "audio.webm" : "audio.bin";
  ...
  const backoff = (attempt) => new Promise((r) => setTimeout(r, 250 * Math.pow(2, attempt) + Math.floor(Math.random() * 100)));
  ...
  for (let attempt = 0; attempt < MAX_TRIES; attempt++) {
    try {
      ...
      if (!resp.ok) {
        if (shouldRetry(resp.status, text) && attempt < MAX_TRIES - 1) { await backoff(attempt); continue; }
      ...
      return json(200, { transcript: data.text || "", meta: { bytes: buf.length, mime: finalMime, model: MODEL, magic: summarizeMagic(magic) } });
    } catch (e) {
      lastStatus = -1; lastText = String(e?.message || e);
      if (attempt < MAX_TRIES - 1) { await backoff(attempt); continue; }
      return json(502, { error: "stt_exception", detail: lastText, meta: { bytes: buf.length, mime: finalMime, model: MODEL } });
    }
  }
  return json(502, { error: "stt_failed", detail: lastText || "Unknown STT failure", meta: { status: lastStatus } });
};

function safeParse(t){ try { return JSON.parse(t); } catch { return null; } }
function summarizeMagic(b){ try { return b.toString("hex").slice(0, 16); } catch { return ""; } }
